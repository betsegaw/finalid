// <copyright file="Cryptography.cs" company="FinalID">
// Copyright (c) FinalID. All rights reserved.
// Licensed under the MIT license. See LICENSE file in the project root for full license information.
// </copyright>

namespace FinalId.Tests
{
    using FinalId.Core;
    using Microsoft.VisualStudio.TestTools.UnitTesting;

    [TestClass]
    public class Cryptography
    {
        [TestInitialize]
        public void Initialize()
        {
            KeyPair.TestKeyPair = new KeyPair("{\"Modulus\":\"\",\"Exponent\":\"AQAB\",\"P\":\"7cRAFm71MrKqNhmbqNYdXJM07caBcQ00pwcJuUwVcp4I5E0l0oAwEU/0SM0byR8CMWlbcbsPd2ToIScruAuZAIKdk2QTN0yL214lOyKTApZOZVBIcI/PG39R3bkVQLJgLR3hQQyYheMTie9jsEKP4617X8+2bGE9IZW4VOvjI9s=\",\"Q\":\"0zmIwHx+THY2lf+CTUFqo2m/lu0wqIVy2PHSIH/8iJyJh40M+eTHO6cCGCtunLVuR613AD60wCvtTluZ1n7MpgZyw63pQGf6mUgKqIzhtU5IrmPWCHDD9WBW8XCtZcvmYdcOF7q9GLwVBSuSz24VhnAJUOzXVmbKrhKsbXeqUHc=\",\"DP\":\"CjDkMlH1LoQb6Y+1bUooHgStOxfMCoVvYU1a7FdHgA8oAGkSGvClnshXhxtLepZaqLWEVt6Wrh5kGK+uHwhYOX5ftKaQcElWoUNqwxsbQ/wfRhZn1HNFhD8zyhYZxBkhKC3krr4Ud4ChIMNi+XYQ5shTxmqHviN41EyvhuxJYws=\",\"DQ\":\"AdmZJwZwHL9UTxAD1iVih7ffY7UEHhW9IcVMb9hvH8svMS69FFs6m30k+Y031MuKAbq1Nh1We/Bmja8js8s8g5++1ZPqXRQMD9efsEY7m9jprg1VzJEgRj/nwwmXmzKj++tkRjTufw72qCrviD2QlrLYggrw/+K6l7e2JudLaJU=\",\"InverseQ\":\"i/AXtXT+DQ/yo5nFcejy+r4ea6+656o3wObgcMMDNrxQ+i7Vzem/D8VRpZcao4wwyetfwHEp849KPym+I1rtdFZWYxxlB/d7FE/PeO8jGH+AKLah9Afqa5GW8tbUHtZ/vJ4v0GC2CI0XmqcbKBAYSxGuTe+avzFXN/dAwHf/US4=\",\"D\":\"\"}");
        }

        [TestMethod]
        public void CanEncryptAndDecrypt()
        {
            string content = "Testing encryption and decryption testing";
            string encryptedContent = Core.KeyPair.TestKeyPair.Encrypt(content);
            string decryptedContent = Core.KeyPair.TestKeyPair.Decrypt(encryptedContent);
            Assert.AreEqual(content, decryptedContent, "The original content and the decrypted content are not equal");
        }

        [TestMethod]
        public void CanSignAndVerify()
        {
            string content = "Testing encryption and decryption testing";
            byte[] signedContent = Core.KeyPair.TestKeyPair.Sign(content);
            bool validated = Core.KeyPair.TestKeyPair.Verify(content, signedContent);
            Assert.IsTrue(validated, "Signature is not validated");
        }
    }
}
